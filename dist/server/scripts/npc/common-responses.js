...
exports.TannerResponses = (npc) => {
    ...
    npc.parser.addCommand('tan')
        ...
        if (!ground.Corpse || !ground.Corpse.length)
            return 'There are no corpses here!';
        ground.Corpse.forEach(corpse => {
            ...
            if (corpseNPC)
                ...
                npc.$$room.removeItemFromGround(corpse);
            if (!corpse.tansFor)
                return;
            npc_loader_1.NPCLoader.loadItem(corpse.tansFor)
                .then(item => {
...
exports.PeddlerResponses = (npc) => {
    npc.parser.addCommand('hello')
        .set('syntax', ['hello'])
        .set('logic', (args, { player }) => {
        if (!npc.peddleItem || !npc.peddleCost) {
            logger_1.Logger.error(new Error(`Peddler at ${npc.x}, ${npc.y} - ${npc.map} does not have a valid peddleItem/peddleCost`));
            return;
        }
    ...
    npc.parser.addCommand('buy')
        ...
        if (npc.distFrom(player) > 2)
            return 'Please move closer.';
        if (player.rightHand)
            return 'Please empty your right hand!';
        if (player.gold < npc.peddleCost)
            return `I can't offer this for free. You need ${npc.peddleCost.toLocaleString()} gold for a ${npc.peddleItem}.`;
        player.loseGold(npc.peddleCost);
        const item = new item_1.Item(npc.rightHand);
        item.regenerateUUID();
        player.setRightHand(item);
        return `Thank you, ${player.name}!`;
    });
};
exports.SmithResponses = (npc) => {
    npc.parser.addCommand('hello')
        .set('syntax', ['hello'])
        .set('logic', (args, { player }) => {
        if (!npc.costPerThousand || !npc.repairsUpToCondition) {
            logger_1.Logger.error(new Error(`Smith at ${npc.x}, ${npc.y} - ${npc.map} does not have a valid costPerThousand/repairsUpToCondition`));
            return;
        }
        if (npc.distFrom(player) > 2)
            return 'Please move closer.';
        if (player.rightHand) {
            ...
            if (missingCondition < 0)
                return 'That item is already beyond my capabilities!';
            const cost = Math.floor(missingCondition / 1000 * cpt);
            if (cost === 0)
                return 'That item is not in need of repair!';
            if (player.gold < cost)
    ...
    npc.parser.addCommand('repairall')
        ...
        const cpt = npc.costPerThousand || 1;
        let totalCosts = 0;
        character_1.AllNormalGearSlots.forEach(slot => {
            const item = lodash_1.get(player, slot);
            if (!item || item.condition > myCondition)
                return;
...
exports.RandomlyShouts = (npc, responses = []) => {
    let ticks = 0;
    let nextTick = lodash_1.random(5, 10);
    npc.$$ai.tick.add(() => {
        ticks++;
        if (ticks >= nextTick) {
            nextTick = lodash_1.random(5, 10);
            ticks = 0;
            let response = lodash_1.sample(responses);
            if (responses.length > 1 && response === npc.$$lastResponse) {
                do {
                    response = lodash_1.sample(responses);
...
exports.AlchemistResponses = (npc) => {
    npc.parser.addCommand('hello')
        .set('syntax', ['hello'])
        .set('logic', (args, { player }) => {
        if (!npc.alchCost || !npc.alchOz) {
            logger_1.Logger.error(new Error(`Alchemist at ${npc.x}, ${npc.y} - ${npc.map} does not have a valid alchCost/alchOz!`));
            return;
        }
        if (npc.distFrom(player) > 2)
            return 'Please move closer.';
    ...
    npc.parser.addCommand('combine')
        ...
        if (!item || item.itemClass !== 'Bottle')
            return 'You are not holding a bottle.';
        if (item.ounces >= npc.alchOz)
        ...
        player.sack.items.forEach((checkItem, i) => {
            if (checkItem.name !== item.name)
                return;
            if (checkItem.ounces + item.ounces > npc.alchOz)
                return;
            const cost = checkItem.ounces * npc.alchCost;
            if (npc.alchCost > cost)
                return;
            player.loseGold(cost);
            indexes.push(i);
        ...
        if (itemsRemoved === 0)
            return 'I was not able to combine any bottles.';
        return `I've taken ${itemsRemoved} bottles from your sack and combined them with the item in your hand. Enjoy!`;
    });
};
exports.BankResponses = (npc) => {
    if (!npc.bankId || !npc.branchId) {
        logger_1.Logger.error(new Error(`Banker at ${npc.x}, ${npc.y} - ${npc.map} does not have a valid bank/branch id`));
        return;
    }
    npc.parser.addCommand('hello')
        .set('syntax', ['hello'])
    ...
    npc.parser.addCommand('deposit')
        ...
        if (res === false)
            return 'That amount of gold is not valid.';
        if (res === 0)
            return 'What, do you think you\'re funny? Get out of line, there are other people to service!';
    ...
    npc.parser.addCommand('withdraw')
        ...
        if (res === false)
            return 'That amount of gold is not valid.';
        if (res === 0)
            return 'Hey, do I look like a charity to you? Get lost!';
...
exports.VendorResponses = (npc, { classRestriction = '' } = {}) => {
    npc.parser.addCommand('hello')
        .set('syntax', ['hello'])
        .set('logic', (args, { player }) => {
        if (npc.vendorItems.length === 0) {
            logger_1.Logger.error(new Error(`Vendor at ${npc.x}, ${npc.y} - ${npc.map} does not have any vendorItems!`));
            return;
        }
    ...
    npc.parser.addCommand('appraise')
        ...
        if (npc.distFrom(player) > 2)
            return 'Please move closer.';
        if (!player.rightHand)
            return 'I cannot give you as value for your hand.';
        const value = player.rightHand.value;
        if (value === 0)
            return 'That item is worthless.';
    ...
    npc.parser.addCommand('sell')
        ...
        const allItems = player.sack.allItems.filter((item, i) => {
            if (item.itemClass.toLowerCase() !== itemtype)
                return false;
        ...
        if (allItems.length === 0)
            return 'You are not carrying any items of that type!';
...
exports.EncrusterResponses = (npc) => {
    npc.parser.addCommand('hello')
        .set('syntax', ['hello'])
        .set('logic', (args, { player }) => {
    ...
    npc.parser.addCommand('encrust')
        ...
        if (npc.distFrom(player) > 0)
            return 'Please move closer.';
        if (!player.rightHand)
            return 'You must hold an item in your right hand!';
        if (player.rightHand.itemClass === 'Corpse')
            return 'That would be disrespectful.';
        if (!player.leftHand || player.leftHand.itemClass !== 'Gem')
            return 'You must hold a gem in your left hand!';
        const cost = player.leftHand.value + player.rightHand.value + 500;
        if (player.gold < cost)
            return `I require ${cost.toLocaleString()} gold for this transaction.`;
...
exports.BaseClassTrainerResponses = (npc, skills) => {
    npc.parser.addCommand('hello')
        .set('syntax', ['hello'])
        .set('logic', (args, { player }) => {
        if (!npc.maxSkillTrain || !npc.trainSkills || !npc.classTrain || !npc.maxLevelUpLevel) {
            logger_1.Logger.error(new Error(`Trainer at ${npc.x}, ${npc.y} - ${npc.map} does not have a valid maxSkillTrain/trainSkills/classTrain/maxLevelUpLevel`));
            return;
        }
        if (npc.distFrom(player) > 0)
            return 'Please move closer.';
    ...
    npc.parser.addCommand('assess')
        ...
        if (!lodash_1.includes(npc.trainSkills, lodash_1.capitalize(skill)))
            return 'I cannot teach you anything about this.';
        ...
        if (player.gold < assessCost)
            return `I require ${assessCost.toLocaleString()} gold for my assessment.`;
        player.loseGold(assessCost);
        if (skillLevel >= maxAssessSkill)
    ...
    npc.parser.addCommand('train')
        ...
        if (npc.distFrom(player) > 0)
            return 'Please move closer.';
        if (player.baseClass !== npc.classTrain)
            return 'I have nothing to teach you.';
        const level = player.level;
        const trainCost = npc.maxLevelUpLevel * 200;
        if (player.gold < trainCost)
            return `I require ${trainCost} gold for my training.`;
        if (level >= npc.maxLevelUpLevel)
            return 'You are too advanced for my teachings.';
    ...
    npc.parser.addCommand('join')
        ...
        if (npc.distFrom(player) > 0)
            return 'Please move closer.';
        if (player.baseClass !== 'Undecided')
            return 'You have already joined a profession.';
    ...
    npc.parser.addCommand('learn')
        ...
        if (learnedSkills.length === 0)
            return 'I cannot currently teach you anything new.';
