...
class CharacterCreator {
    ...
    static getCustomizedCharacter({ allegiance, sex, name }) {
        ...
        if (!name)
            name = '';
        if (!lodash_1.includes(this.validAllegiances(), char.allegiance)) {
            char.allegiance = 'None';
        }
        if (!lodash_1.includes(['Male', 'Female'], char.sex)) {
            char.sex = 'Male';
        }
        char.name = lodash_1.capitalize(lodash_1.truncate(name.replace(/[^a-zA-Z]/g, ''), { length: 20, omission: '' }));
...
CharacterCreator.alleiganceMods = {
    None: {},
...
CharacterCreator.allegianceDesc = {
