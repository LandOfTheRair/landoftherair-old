...
class MoveHelper {
    static move(player, { room, gameState, x, y }) {
        ...
        const denseTiles = gameState.map.layers[maplayer_1.MapLayer.Walls].data;
        ...
        const denseObjects = gameState.map.layers[maplayer_1.MapLayer.DenseDecor].objects;
        const interactables = gameState.map.layers[maplayer_1.MapLayer.Interactables].objects;
        ...
        const getNumStepsSuccessful = (trySteps) => {
            ...
            for (i; i < trySteps.length; i++) {
                ...
                if (nextTile === 0) {
                    const object = lodash_1.find(denseCheck, { x: (player.x + step.x) * 64, y: (player.y + step.y + 1) * 64 });
    ...
    static handleInteractable(room, player, obj) {
        switch (obj.type) {
    ...
    static handleTeleport(room, player, obj) {
