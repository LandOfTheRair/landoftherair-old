...
export class DailyKillMiners extends Quest {
  ...
  public static updateProgress(player: Player, questOpts: any = {}): void {
    ...
    if(structure.kills <= this.killsRequired) {
      player.sendQuestMessage(this, `You have killed ${structure.kills}/${this.killsRequired} miner threats.`);
    }
  ...
  public static incompleteText(player: Player): string {
    ...
    return `Thanks for offering your help. You have to kill ${this.killsRequired - kills} more miner threats.`;
  }
  ...
  public static givePlayerRewards(player: Player): void {
    ...
    player.$$room.partyManager.giveAccountSilver(player.username, 3);

    const gainedResetPoints = player.skillTree.canGainResetPoints ? 2 : 0;
    player.skillTree.gainResetPoints(gainedResetPoints);
    player.sendClientMessage(`You received 500,000 XP, 25,000 gold, 3 silver and ${gainedResetPoints} RP!`);
