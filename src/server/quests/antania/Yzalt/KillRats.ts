...
export class KillRats extends Quest {
  ...
  public static updateProgress(player: Player, questOpts: any = {}): void {
    ...
    if(structure.kills <= this.killsRequired) {
      player.sendQuestMessage(this, `You have killed ${structure.kills}/${this.killsRequired} rats.`);
    }
  ...
  public static incompleteText(player: Player): string {
    ...
    return `By my records, you have to kill ${this.killsRequired - kills} rats yet!`;
  }
  ...
  public static givePlayerRewards(player: Player): void {
    ...
    player.sendClientMessage('You received 50,000 XP and 20,000 gold!');
  }
}
