...
export class KillZombies extends Quest {
  ...
  public static updateProgress(player: Player, questOpts: any = {}): void {
    ...
    if(structure.kills <= this.killsRequired) {
      player.sendQuestMessage(this, `You have killed ${structure.kills}/${this.killsRequired} zombies.`);
    }
  ...
  public static incompleteText(player: Player): string {
    ...
    return `By my records, you have to kill ${this.killsRequired - kills} zombies yet!`;
  }
  ...
  public static givePlayerRewards(player: Player): void {
    this.rewardPlayerGold(player, 20000);
    player.gainExp(20000);
    player.sendClientMessage('You received 20,000 XP and 20,000 gold!');
