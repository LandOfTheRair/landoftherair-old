...
export class KillSpooks extends Quest {
  ...
  public static updateProgress(player: Player, questOpts: any = {}): void {
    ...
    if(structure.kills <= this.killsRequired) {
      player.sendQuestMessage(this, `You have killed ${structure.kills}/${this.killsRequired} spooks.`);
    }
  ...
  public static incompleteText(player: Player): string {
    ...
    return `By my records, you have to kill ${this.killsRequired - kills} spooks yet!`;
  }
  ...
  public static givePlayerRewards(player: Player): void {
    ...
    player.sendClientMessage('You received 250,000 XP and 30,000 gold!');
  }
}
